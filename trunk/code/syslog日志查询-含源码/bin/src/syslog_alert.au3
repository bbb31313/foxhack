...
$Form2 = GUICreate("SYSLOG-alert记录查询 BY:foxhack E-mail:foxhack@qq.com QQ:278563291", 1090, 700, 100, 10)
	...
	Switch $nMsg
		...
		Case $Button1
			...
			If  Not StringLen($str_text)  Then
				$String="select * from alert where received between '"&$str_date1&"' and '"&$str_date2&"' order by received desc "
				first($string)
			Else
				$string="select * from alert where message like '%"&$str_text&"%' and received between '"&$str_date1&"' and '"&$str_date2&"' order by received desc "
				;MsgBox(0,"",$string)
				first($string)
			EndIf
			_ReduceMemory(@AutoItPID)
